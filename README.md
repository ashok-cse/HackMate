...
- **Customize the agent:** Outbound calls set LLM dynamic variables **`{{greetings}}`**, **`{{hackathon_name}}`**, and **`{{questions}}`** (see `retellLlmDynamicVariables`). **`participant_id`** is attached in call **`metadata`** only (for webhooks), not in the LLM variable set.
